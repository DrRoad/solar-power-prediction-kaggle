path <- "/Users/sebastianmontero/Desktop/R Group/Shiny"
setwd(path)
...
### Add filter to chose by day, month or year ###
...
### Add slider to choose between num of stations ###
...
ggplot(total_by_station, aes(y=Total_Value, x=fct_infreq(Station))) +
  geom_bar(stat="identity", fill = "#FFD700") +
  coord_cartesian(ylim=c(7.5e+10, 1e+11), expand = F) +
  coord_flip() +
  theme_classic() +
  geom_label(aes(label = Total_Value))+
